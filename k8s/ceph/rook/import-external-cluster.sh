...
function applyClusterNamespace() {
...
function applyECRBDStorageClass() {
...
function applyCephFSStorageClass() {
...
if [ -n "$RBD_POOL_NAME" ]; then
  if [ -n "$RBD_METADATA_EC_POOL_NAME" ]; then
    applyECRBDStorageClass
  else
    applyRBDStorageClass
  fi
fi
if [ -n "$CEPHFS_FS_NAME" ] && [ -n "$CEPHFS_POOL_NAME" ]; then
  applyCephFSStorageClass
fi
